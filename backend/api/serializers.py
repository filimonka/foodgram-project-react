...
class RecipeSerializer(serializers.ModelSerializer):
    ...
    ingredients = RecipeIngredientSerializer(many=True, source='recipe_ingredient', required=True)
    ...
    def validate_tags(self, value):
        if type(value) != list:
            raise ValidationError('Поле тэг передано в неподходящем формате. Ожидаемый формат: list')
        tag_id_list = Tag.objects.all().values_list('id', flat=True)
        for tag in value:
            if tag not in tag_id_list:
                raise ValidationError(f'Тэг с id {tag} не найден, проверьте переданные данные')
        return value
    ...
    def update(self, instance, validated_data): ### Doesn't work
